...
def myloss(y_true,y_pred):
    ...
    class_weights = tf.constant([[0.5, 3.0, 3.0,1.0,3.0,3.0,1.0,3.0,3.0,3.0,1.0]])
...
model.compile(optimizer=Adam(1e-4), loss='myloss', metrics=[acc])
model.fit_generator(gen_train.generator, steps_per_epoch=gen_train.steps, epochs=1, validation_data=(X_valid, y_valid))
model.compile(optimizer=Adam(1e-5), loss='myloss', metrics=[acc])
model.fit_generator(gen_train.generator, steps_per_epoch=gen_train.steps, epochs=1, validation_data=(X_valid, y_valid))
model.compile(optimizer=Adam(1e-6), loss='myloss', metrics=[acc])
model.fit_generator(gen_train.generator, steps_per_epoch=gen_train.steps, epochs=2, validation_data=(X_valid, y_valid))
model.compile(optimizer=Adam(1e-7), loss='myloss', metrics=[acc])
...
model.save('round2/model_nasnet3.h5')
