...
model.compile(optimizer='adam', loss='focal_loss1', metrics=['acc'])

model.compile(optimizer=Adam(1e-4), loss='focal_loss1', metrics=[acc])
model.fit_generator(gen_train.generator, steps_per_epoch=gen_train.steps, epochs=2, validation_data=(X_valid, y_valid))
model.compile(optimizer=Adam(1e-5), loss='focal_loss1', metrics=[acc])
model.fit_generator(gen_train.generator, steps_per_epoch=gen_train.steps, epochs=5, validation_data=(X_valid, y_valid))
model.compile(optimizer=Adam(1e-6), loss='focal_loss1', metrics=[acc])
model.fit_generator(gen_train.generator, steps_per_epoch=gen_train.steps, epochs=15, validation_data=(X_valid, y_valid))
model.compile(optimizer=Adam(1e-7), loss='focal_loss1', metrics=[acc])
...
with open('round2/focal_2_0.5.csv','w') as f:
