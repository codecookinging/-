...
def read_img(index):
    return index, cv.resize(cv.imread(img_path[index]),(width,width))

def read_img1(index):
    return index, cv.resize(cv.imread(img_path1[index]),(width,width))
...
test_path='../round1_test_a'
test_img_name=os.listdir(test_path)
n_test=len(test_img_name)

def read_img_test(index):
    return index, cv.resize(cv.imread(os.path.join(test_path,test_img_name[index])),(width,width))
...
df.to_csv('../pred07.csv',index=None, header=None)
print('csv saved!')

model.save('model_nasnet07.h5')
