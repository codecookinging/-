...
class BatchPreprocessor(object):
    ...
    def __init__(self, dataset_file_path, num_classes, output_size=[227, 227], horizontal_flip=False, shuffle=False,
        ...
        for line in lines:
            items = line.split('|')  # use \ to split label and filename
    ...
    def next_batch(self, batch_size):
        ...
        paths = self.images[self.pointer:(self.pointer + batch_size//2)]
        labels = self.labels[self.pointer:(self.pointer + batch_size//2)]

        # Update pointer
        self.pointer += batch_size//2
        ...
        for i in range(len(paths)):
            ...
            if self.multi_scale is None:
                ...
                img1=cv2.resize(img1,(self.output_size[0], self.output_size[0]))
            ...
            img1 -= np.array(self.mean_color)

            # print(img)
            images[i] = img

            images[i+len(paths)]=img1

        # Expand labels to one hot encoding
        one_hot_labels = np.zeros((batch_size, self.num_classes))
        for i in range(len(labels)):
            one_hot_labels[i][labels[i]] = 1
            one_hot_labels[i+len(labels)][labels[i]]=1



        # np.savetxt('resut.txt',images) 4 array list xie buliao txt tamade buneng da zhong wen
