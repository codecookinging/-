...
def main(_):
    ...
    FLAGS.learning_rate = tf.train.exponential_decay(FLAGS.learning_rate, global_step, decay_steps=2000, decay_rate=0.5, staircase=True)
    train_op = model.optimize(FLAGS.learning_rate, train_layers,global_step)
    '''
    加了 可变学习率 
    '''
    ...
    train_preprocessor = BatchPreprocessor(dataset_file_path=FLAGS.training_file, num_classes=FLAGS.num_classes,
                                           output_size=[224, 224], horizontal_flip=True, shuffle=True, multi_scale=None)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            while step < train_batches_per_epoch:
                ...
                trainacc=sess.run(accuracy, feed_dict={x: batch_xs, y: batch_ys, dropout_keep_prob: FLAGS.dropout_keep_prob})
                trainloss = sess.run(loss,feed_dict={x: batch_xs, y: batch_ys, dropout_keep_prob: FLAGS.dropout_keep_prob})

                print('trainaccuary',trainacc,trainloss)
            ...
            for _ in range(val_batches_per_epoch):
                ...
                valoss = sess.run(loss, feed_dict={x: batch_tx, y: batch_ty, dropout_keep_prob: 1.})
                test_acc += acc
                test_count += 1
                print("{} one_batch Accuracy = {:.4f}".format(datetime.datetime.now(), acc))
                print("valloss",valoss)
            test_acc /= test_count
            s = tf.Summary(value=[
